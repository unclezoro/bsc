...
func (p *statePrefetcher) Prefetch(block *types.Block, statedb *state.StateDB, cfg vm.Config, interrupt *uint32) {
	...
	for idx, tx := range transactions {
		...
		sortTransactions[threadIdx] = append(sortTransactions[threadIdx], tx)
	}
...
func (p *statePrefetcher) MinerPrefetch(header *types.Header, txs *types.TransactionsByPriceAndNonce,
	statedb *state.StateDB, cfg vm.Config, exit chan struct{}) {
	var (
		signer = types.MakeSigner(p.config, header.Number)
	)
	if statedb == nil {
	...
	for i := 0; i < prefetchThread; i++ {
		newStatedb := statedb.Copy()
		go func(s *state.StateDB) {
			for {
				select {
				case tx := <-txsChan:
					gaspool := new(GasPool).AddGas(header.GasLimit)
					blockContext := NewEVMBlockContext(header, p.bc, nil)
					evm := vm.NewEVM(blockContext, vm.TxContext{}, s, p.config, cfg)
					// Convert the transaction into an executable message and pre-cache its sender
					msg, err := tx.AsMessage(signer)
					if err != nil {
						continue
					}
					s.Prepare(tx.Hash(), header.Hash(), i)
					if err := precacheTransaction(msg, p.config, gaspool, s, header, evm); err != nil {
						continue
					}
				case <-closeCh:
					return
