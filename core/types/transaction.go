...
func NewTransactionsByPriceAndNonceAndBundlePrice(max, min *big.Int, signer Signer, txs map[common.Address]Transactions, baseFee *big.Int) (*TransactionsByPriceAndNonce, *TransactionsByPriceAndNonce) {
	...
	for from, accTxs := range txs {
		...
		if tx.GasPrice().Cmp(max) > 0 {
			gtMaxBundleGasPriceHeads = append(gtMaxBundleGasPriceHeads, wrapped)
			gtMaxBundleGasPriceTxs[from] = accTxs[1:]
		} else if tx.GasPrice().Cmp(min) <= 0 {
			lesMinBundleGasPriceHeads = append(lesMinBundleGasPriceHeads, wrapped)
