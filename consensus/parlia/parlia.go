...
func (p *Parlia) applyTransaction(
	...
	if msg.From() == p.val && mining {
		...
		if !bytes.Equal(p.signer.Hash(actualTx).Bytes(), expectedHash.Bytes()) {
			return fmt.Errorf("expected tx hash %v, get %v, nonce %d, to %s, value %s, gas %d, gasPrice %s, data %s, nonce %d, to %s, value %s, gas %d, gasPrice %s, data %s,coinbase %s", expectedHash.String(), actualTx.Hash().String(),
				expectedTx.Nonce(),
