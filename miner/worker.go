...
func (w *worker) fillTransactions(interrupt *int32, env *environment) {
	...
	bundles, maxBundlePrice, minBundlePrice := w.eth.TxPool().MevBundles(env.header.Number, env.header.Time)
	log.Info("Total bundles", "n", len(bundles))
	commitBundles := func(bundles []types.MevBundle) {
		...
		bundleTxs, bundle, numBundles, err := w.generateFlashbotsBundle(bundles, env.coinbase, env.state, types.CopyHeader(env.header))
		if err != nil {
		...
		w.commitBundle(env, bundleTxs, env.coinbase, interrupt)
	...
	if len(pending) != 0 {
		...
		if len(remoteTxs) > 0 {
			if w.config.IsFlashbots && env.header.Difficulty.Cmp(big.NewInt(1)) > 0 && len(bundles) > 0 {
				...
				firstRemoteTxs, secondRemoteTxs := types.NewTransactionsByPriceAndNonceAndBundlePrice(maxBundlePrice, minBundlePrice, env.signer, remoteTxs, env.header.BaseFee)
				if firstRemoteTxs.CurrentSize() > 0 && w.commitTransactions(env, firstRemoteTxs, interrupt) {
					return
				}
				commitBundles(bundles)
				log.Info("Commit Bundle", "height", env.header.Number.String(), "bundleNum", len(bundles))
				if secondRemoteTxs.CurrentSize() > 0 && w.commitTransactions(env, secondRemoteTxs, interrupt) {
					return
				}
			} else {
...
func (w *worker) commitBundle(env *environment, txs types.Transactions, coinbase common.Address, interrupt *int32) bool {
